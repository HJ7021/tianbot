...
PurePursuit::PurePursuit() {
    ...
    pravite_nh.param("perpendicular_tolerance", perpendicular_tolerance, 1.4); // 横向容忍程度 
    ...
    path_sub_ = nh_.subscribe("/tianracer/move_base/TebLocalPlannerROS/global_plan", 1, &PurePursuit::pathCallback, this); //与move_base连用
...
void PurePursuit::odomCallback(const nav_msgs::Odometry::ConstPtr &odomMsg) {  //车辆当前位置和目标点之间的欧几里得距离
    ...
    line_end.x = -9.9;
    line_end.y = 1.5;
        if (crossesLine(line_start, line_end, robot_position))
    {
        lap_count_++;
