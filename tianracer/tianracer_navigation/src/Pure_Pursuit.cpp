...
PurePursuit::PurePursuit() {
    ...
    pravite_nh.param("Lfw", Lfw, 1.3); // 前视距离
...
void PurePursuit::PurePursuitCallback(const std_msgs::Int32::ConstPtr& msg) {
    ...
    // ROS_INFO("PurePursuitCallback: %d", Pure_Pursuit_out);
...
//判断路径点是否在机器人的前方
{
    ...
    if(car_car2wayPt_x >0) /*is Forward WayPt*/  //前方
        return true;
    else
        return false;
}
...
geometry_msgs::Point PurePursuit::get_odom_car2WayPtVec(const geometry_msgs::Pose& carPose)
{
    ...
    if(!goal_reached_){   //如果尚未到达目标点
        ...
        ROS_INFO("repate: %d", count_index_repate);
...
void PurePursuit::controlLoopCallback(const ros::TimerEvent&)  //控制车辆
{
    ...
    // ROS_INFO("controlLoopCallback: %d", Pure_Pursuit_out);
        ...
        //        cout<<eta<<endl;
            if(foundForwardPt_)
            {
                ...
                {
                    this->velocity = this->reference_v;   //设置为参考速度
                }
            }
        }
